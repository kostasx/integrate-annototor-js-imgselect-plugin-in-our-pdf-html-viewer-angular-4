function annotatorImageSelect(options) {
  ...
  return {
    ...
    start: function (app) {
      ...
      // LOAD STORED ANNOTATTIONS VIA LocalStorage
      // console.log(app.annotations.store.create);
      // console.log(options.pageNum);
      let storageKey = "page" + options.pageNum;
      let storedAnnotations = localStorage.getItem(storageKey);
      storedAnnotations = JSON.parse(storedAnnotations);
      if (storedAnnotations) {
        console.log(storedAnnotations.length, storedAnnotations);
        console.log("Stored annotation found for page #" + options.pageNum);
        if (storedAnnotations.length > 1) {

          console.log("Multiple annotations found on page #" + options.pageNum);
          storedAnnotations.map((ann, idx) => {
            imgselect_utils.drawImageHighlight(ann);
          });

        } else {

          console.log("Single annotation found on page #" + options.pageNum);
          imgselect_utils.drawImageHighlight(storedAnnotations[0]);
