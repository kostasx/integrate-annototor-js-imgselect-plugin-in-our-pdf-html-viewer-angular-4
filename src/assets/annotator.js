...
var Viewer = exports.Viewer = Widget.extend({
    ...
    _onDeleteClick: function (event) {
		...
		var pageEntries = Object.keys(localStorage); 
		pageEntries.some((currentPageEntry)=>{
			var entries = JSON.parse(localStorage[currentPageEntry]);
			var hasEntry = entries.some((pageEntry,idx)=>{
				if ( pageEntry.uuid == item.uuid ){
			...
			if ( entries.length > 0 ){
				localStorage.setItem( currentPageEntry, JSON.stringify( entries ) );
			} else {
				localStorage.removeItem( currentPageEntry );
			}
