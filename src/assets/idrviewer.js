/*! IDRViewer - v1.5.3 | Copyright 2017 IDRsolutions */
!(function () {
  "use strict";
  var a,
    b,
  ...
  l.setup = function (m) {
    ...
    for (
      ...
      z.style.minWidth = "100%",
      z.style.lineHeight = "normal",
      b.appendChild(z),
      g = document.createElement("div"),
      g.id = "overlay",
      g.setAttribute(
        "style",
        "width: 100%; height: 100%; position: absolute; z-index: 10; visibility: hidden;"
      ),
      z.appendChild(g),
      J.setup(),
      ...
      (B.id = "page" + v),
        B.setAttribute(
          "style",
          "width: " + f[v - 1][0] + "px; height: " + f[v - 1][1] + "px;"
        ),
        (B.className = "page"),
        a.appendChild(B),
        (t[v] = B),
    ...
    for (var E in m) m.hasOwnProperty(E) && (D[E] = m[E]);
    (D.page = n), l.fire("ready", D);
  };
  var A = function (a, b) {
    var c = {};
    return (
      ...
      (c.unload = function () {
        C.getState(b) === B.LOADED &&
          (C.setState(b, B.HIDDEN), (a.firstChild.style.display = "none"));
      }),
      (c.load = function () {
        var c = C.getState(b);
        if (
          (c === B.HIDDEN &&
            (C.setState(b, B.LOADED), (a.firstChild.style.display = "block")),
            c === B.UNLOADED)
        )
              ...
              (h.onload = function () {
                if (h.status >= 200 && h.status < 400) {
                  ...
                  if ((j && f.addEventListener("load", e), x)) {
                    ...
                    k
                      ? f.setAttribute("data", x + k)
                      : ((k = f.getAttribute("src")),
                        k &&
                        k.indexOf("base64") === -1 &&
                        f.setAttribute("src", x + k));
                  }
                  ...
                  o &&
                    (o.parentNode.removeChild(o),
                      z || (document.head.appendChild(o), (z = !0))),
                  ...
                  for (
                    ...
                    q = d.querySelectorAll("[data-action-type='GoTo']"),
                    r = 0;
                    r < q.length;
                    r++
                  )
                    q[r].onclick = p;
                  a.appendChild(d), j || e(), l.fire("pageload", { page: b });
                } else g();
              }),
              (h.onerror = g);
            try {
    ...
    C = (function () {
      ...
      (d.setup = function () {
        c = o;
        for (var a = 1; a <= o; a++) k[a] = B.UNLOADED;
      }),
        ...
        m = function () {
          if ((s[a].load(), h < g))
            for (
              var c = 1;
              c < f / 2 &&
              (n(a - c) && (s[a - c].isLoaded() || s[a - c].load()), h !== g) &&
              (n(a + c) && (s[a + c].isLoaded() || s[a + c].load()), h !== g);
              c++
            );
          for (var d = 1, j = o; i + h > f;)
            a - d > j - a
              ? (s[d].isLoaded() && s[d].unload(), d++)
              : (s[j].isLoaded() && s[j].unload(), j--);
          b = setTimeout(m, e);
        },
        n = function (a) {
          return a >= 1 && a <= o;
      ...
      return (
        (d.setPage = function (c, d) {
          (a = c), d && s[c].load(), clearTimeout(b), (b = setTimeout(m, e));
        }),
        (d.stopLoading = function () {
          clearTimeout(b), (b = setTimeout(m, e));
        }),
        d
      );
    })(),
      ...
      return (
        (c.updateLayout = function () {
          ...
          i > g ? (h = (i - g) / 2) : (i = g),
            ...
            (a.style.height = i + "px");
          for (var j = 1; j <= o; j++)
            (t[j].style.marginLeft = d + "px"),
              (t[j].style.marginTop = h + "px");
        }),
        (c.isLastPage = function (a) {
          return a === o;
        }),
        (c.getZoomBounds = function () {
          return { width: f[n - 1][0], height: f[n - 1][1] };
        }),
    ...
    G = (function () {
      ...
      var e = function (a) {
        ...
        if (
          (P.addClass(t[a], "current"),
            c(a) && P.addClass(t[a + 1], "current"),
            1 == a && (a = 0),
            a + 2 <= o &&
            (P.addClass(t[a + 2], "next"),
              a + 3 <= o && P.addClass(t[a + 3], "next")),
            a - 1 > 0 &&
            (P.addClass(t[a - 1], "prev"),
              a - 2 > 0 && P.addClass(t[a - 2], "prev")),
            a + 4 <= o)
        )
          for (b = a + 4; b <= o; b++) P.addClass(t[b], "after");
        if (a - 3 > 0) for (b = a - 3; b > 0; b--) P.addClass(t[b], "before");
      };
      return (
        (d.updateLayout = function () {
          ...
          for (var v = 2; v <= o; v += 2)
            (t[v].style.marginLeft = j + "px"),
              (t[v].style.marginTop = r + "px"),
              v < o &&
              ((t[v + 1].style.marginLeft = k + "px"),
                (t[v + 1].style.marginTop = s + "px"));
        }),
        ...
        (d.getZoomBounds = function () {
          ...
          return { width: 2 * Math.max(b, d), height: Math.max(e, g) };
        }),
    ...
    H = (function () {
      ...
      var g = function () {
        ...
        h = function () {
          var a, b;
          if (t[1].getBoundingClientRect().top > 0) E(1);
          else
            for (a = 1; a <= o; a++) {
        ...
        i = function () {
          ...
          var a,
            ...
            f = function (a) {
              return (
                (c = t[a].getBoundingClientRect()), c.bottom > 0 && c.top < d
              );
            };
          for (a = n - 1; a >= 1 && f(a); a--) e.push(a);
          for (a = n + 1; a <= o && f(a); a++) e.push(a);
        };
      return (
        (a.goToPage = function (a, c) {
          ...
          (b.scrollTop = t[a].offsetTop - u / 2 + d * m), E(a), i();
        }),
    ...
    J = (function () {
      ...
      return (
        (f.Default = ""),
        j
          ? ((f.Grab = "-moz-grab"), (f.Grabbing = "-moz-grabbing"))
          : k
            ? ((f.Grab = "-webkit-grab"), (f.Grabbing = "-webkit-grabbing"))
            : ((f.Grab = "all-scroll"), (f.Grabbing = "all-scroll")),
  ...
  l.setSelectMode = function (a) {
    if (w) {
      ...
      a == l.SELECT_SELECT ? J.enableTextSelection() : J.enablePanning(),
        l.fire("selectchange", { type: a });
    } else J.setDefaultMode(a);
  };
  var K,
    L = function (a, b, c, d, e) {
      ...
      return (
        (f = e
          ...
          : "translateX(" + b + "px) translateY(" + c + "px) scale(" + d + ")"),
        D.getTransform(f)
      );
    },
    M = function (a) {
      ...
      for (var u = 0; u < o; u++)
        (t[u + 1].style.width = Math.floor(f[u][0] * m) + "px"),
          (t[u + 1].style.height = Math.floor(f[u][1] * m) + "px");
      (b.scrollTop = j * i),
        p++ ,
        p % 2 === 1 && M(),
        l.fire("zoomchange", {
  ...
  (l.goToPage = function (a, b) {
    ...
    (l.setLayout = function (a) {
      w
        ? (c.unload(),
          P.removeClass(b, "layout-" + c.toString()),
          a == l.LAYOUT_PRESENTATION
            ? (c = F)
            : a == l.LAYOUT_MAGAZINE
              ? (c = G)
              : a == l.LAYOUT_CONTINUOUS && (c = H),
          c.setup(n),
          P.addClass(b, "layout-" + c.toString()),
          M(IDRViewer.ZOOM_AUTO),
          c.goToPage(n),
          l.fire("layoutchange", { layout: a }))
        : (j = a);
    }),
  ...
  (l.on = function (a, b) {
    v[a] || (v[a] = []), v[a].indexOf(b) === -1 && v[a].push(b);
  }),
    (l.off = function (a, b) {
      if (v[a]) {
        var c = v[a].indexOf(b);
  ...
  if (window) {
    ...
    window.Draggable = function (a) {
      ...
      function c() {
        (e.isMoving = !1), (Q = e.element.parentNode);
      }
      function d(a) {
        e.isMoving !== !1 &&
          (a.preventDefault ? a.preventDefault() : (a.returnValue = !1),
            (a = a || window.event),
            (e.newPos.left =
              (a.clientX || a.touches[0].pageX) - e.mousePos.x + e.offsets.left),
            (e.newPos.top =
              (a.clientY || a.touches[0].pageY) - e.mousePos.y + e.offsets.top),
            (e.element.parentNode.style.top = e.newPos.top + "px"),
            (e.element.parentNode.style.left = e.newPos.left + "px"));
      }
      (this.element = a),
        (this.offsets = {}),
        (this.mousePos = {}),
      ...
      if (((this.element.onmousemove = null), this.html.addEventListener))
        if (this.isTouch) {
          ...
          this.element.addEventListener("mousedown", b),
            this.html.addEventListener("mousemove", d),
            this.html.addEventListener("mouseup", c);
      else
        this.html.attachEvent &&
          (this.html.attachEvent("onmousedown", b),
            this.html.attachEvent("onmousemove", d),
            this.html.attachEvent("onmouseup", c));
      a.onclick = function () {
        e.offsets.left === e.newPos.left &&
          e.newPos.top === e.offsets.top &&
          (toggleAnnotation(e.element.parentNode.id), (e.isMoving = !1));
      };
    };
  }
